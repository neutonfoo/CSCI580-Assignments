...
class TextureMap:
    def __init__(self, texture_map_file_name: str):
        print(f"Texture Map file: '{texture_map_file_name}'.")

        self.im = Image.open(texture_map_file_name)

        self.width = self.im.size[0]
        self.height = self.im.size[1]

    def get_color(self, u: float, v: float):
        return np.array(self.im.getpixel((int(self.width * u), int(self.height * v))))
...
class Canvas:
    def __init__(
        self, scene_file_name: str, lighting_mode: LightingMode, texture_map: TextureMap
    ):
    ...
    def draw_triangle(self, triangle: Triangle, material: Material):
            ...
            for x in range(max(0, x_min), min(self.width, x_max + 1)):
                ...
                if alpha >= 0 and beta >= 0 and gamma >= 0:
                    ...
                    if z < self.z_buffer[y][x]:
                        ...
                        # Load texture
                        interpolated_t_over_z = np.array(
                            alpha * triangle.vertices[0].t / triangle.vertices[0].v[2]
                            + beta * triangle.vertices[1].t / triangle.vertices[1].v[2]
                            + gamma * triangle.vertices[2].t / triangle.vertices[2].v[2]
                        )

                        z_at_pixel = 1 / np.array(
                            alpha * (1 / triangle.vertices[0].v[2])
                            + beta * (1 / triangle.vertices[1].v[2])
                            + gamma * (1 / triangle.vertices[2].v[2])
                        )

                        interpolated_t_over_z *= z_at_pixel

                        texture_color = self.texture_map.get_color(
                            interpolated_t_over_z[0], interpolated_t_over_z[1]
                        )
...
def main():
    ...
    # canvas = Canvas(
    #     "scene.json", lighting_mode=LightingMode.GOURAUD, texture_map=texture_map
    # )
    # canvas = Canvas(
    #     "scene.json", lighting_mode=LightingMode.PHONG, texture_map=texture_map
    # )

    texture_map = TextureMap("pride.jpg")
    canvas = Canvas(
        "scene.json", lighting_mode=LightingMode.GOURAUD, texture_map=texture_map
    )
